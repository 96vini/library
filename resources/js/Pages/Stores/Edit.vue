<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editing {{ store.name }}</h2>
        </template>
                    ...
                    <div class="p-6 text-gray-900">
                        <form @submit.prevent="submitForm">
                            <div class="grid grid-cols-1 gap-4">
                                ...
                                <div class="col-span-2">
                                    <label for="name" class="block text-sm font-medium text-gray-700">Current cover</label>
                                    <img :src="store.image_url" :alt="store.name" class="h-[300px] rounded">
                                </div>
                                <div class="col-span-2">
                                ...
                                <div class="col-span-2">
                                    ...
                                    <select multiple id="countries_multiple" v-model="form.books" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                        <option disabled>Press ⌘ or shift to select multiples</option>
                                        <!-- Loop through all books -->
                                        <option v-for="book in books" :key="book.id" :value="book.id" v-bind:selected="form.books.includes(book.id)">{{ book.name }}</option>
                                    </select>
...
const { props } = usePage();
const { store, books } = props;

const form = {
    id: store.id,
    name: store.name,
    address: store.address,
    image: store.image_url ? store.image_url : '',
    active: store.active == 1 ? true : false,
    books: store.books.map(book => book.id),
};
...
const submitForm = async () => {
    try {
        await router.post(route('stores.update'), form)
            .then(() => {
