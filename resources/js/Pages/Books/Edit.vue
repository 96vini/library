<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editing {{ book.name }}</h2>
        </template>
                        ...
                        <form @submit.prevent="submitForm">
                            <div class="grid grid-cols-1 gap-4">
                                ...
                                <div class="col-span-2">
                                    ...
                                    <img :src="book.image_url" :alt="book.name" class="h-[300px] rounded">
                                </div>
                                <div class="col-span-2">
                                    <label for="name" class="block text-sm font-medium text-gray-700">Replace Cover</label>
...
const submitForm = async () => {
    try {
        await router.post(route('books.update'), form)
            .then(() => {
                router.visit(route('books'));
            });
    } catch (error) {
        console.error(error);
    }
